...
<h2 id="1">fido 的基本概念</h2> 
   <h3 id="1.1">fido 产生之前的情况</h3>fido产生之前，我们能想到认证用户的方式就是：密码认证。但是，随着模式识别的技术和传统密码暴露的诸多缺陷问题，之后，人们想到了指纹，声音，脸型等认证方式，来代替密码的认证方式，然而纷乱的方式也存在巨大的问题（之后阐述）
   <h3 id="1.2">fido的概念</h3>
   fido的基本概念：fido实际一种协议，这是fido联盟起草的这份协议。
   </br>fido联盟：FIDO Alliance，成立于2012年7月。FIDO的会员包括Nok Nok Labs、Google、BlackBerry、ARM、英特尔、PayPal、Lenovo、MasterCard、三星、VISA、Synaptics、RSb、微软等等。支付宝也于今年9月份左右加入的FIDO组织。
...
<h2 id="2">fido 的产生之前认证方式的弊端</h2>
  ...
  * 传统的弊端，如图所示：
   ...
   *  其实在fido产生之前，人们已经有各种解决密码这种比较简单的认证方式了，比如指纹，虹膜，脸型，IC卡等林林总总的认证方式
       * 截图如下所示：
       * ![](1.1.2.png)
       * 这样的方式确实可以解决密码认证的各种问题，但是由于认证方式的多样性的产生，导致了不同的设备，不同服务器都要进行开发,组合起来之后，会出现方式和设备在数量和开发难度上的问题：
       * 这种认证方式的带来的问题的：
       * ![](1.1.3.png)
...
方式和能力并设置认证策略 
   ...
   *  验证了用户的在认证过程中的真实身份。
   *  让攻击者在客户端和服务器传输的过程中，攻击无效
   *  服务器不再存储用户名和密码的键值对,,就算遭到攻击，数据库泄露，也不会造成损失
